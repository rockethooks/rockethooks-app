...
- **Performance Metrics**: Show any time-series KPI data
  ...
  argTypes: {
    ...
    color: {
      control: { type: 'select' },
      options: ['text-primary', 'text-green-600', 'text-blue-600', 'text-red-600', 'text-yellow-600', 'text-purple-600'],
...
const responseTimeData = [145, 152, 138, 165, 142, 159, 146, 171, 155, 148, 163, 139]
const uptimeData = [99.2, 99.8, 99.5, 99.9, 99.1, 99.7, 99.6, 99.3, 99.8, 99.4, 99.9, 99.6]
const errorRateData = [2.1, 1.8, 2.5, 1.2, 1.9, 0.8, 1.5, 2.3, 1.1, 1.7, 0.9, 1.3]
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Sparkline data={responseTimeData} width={60} height={20} color="text-blue-600" />
      ...
      <div className="space-y-2">
        ...
        <Sparkline data={responseTimeData} width={100} height={32} color="text-green-600" gradient />
      </div>

      <div className="space-y-2">
        <h4 className="font-medium text-sm">Large (dashboard)</h4>
        <Sparkline data={responseTimeData} width={160} height={48} color="text-purple-600" gradient showTooltip />
      </div>
    </div>
  ),
  parameters: {
    ...
    <div className="grid grid-cols-2 gap-6">
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Sparkline data={uptimeData} color="text-green-600" width={80} height={24} />
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium">Response Time</span>
          <Sparkline data={responseTimeData} color="text-blue-600" width={80} height={24} />
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium">Error Rate</span>
          <Sparkline data={errorRateData} color="text-red-600" width={80} height={24} />
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Sparkline data={trafficData} color="text-purple-600" width={80} height={24} />
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium">CPU Usage</span>
          <Sparkline data={volatileData} color="text-yellow-600" width={80} height={24} />
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium">Memory</span>
          <Sparkline data={[65, 67, 69, 71, 68, 72, 70, 74, 71, 73, 69, 71]} color="text-indigo-600" width={80} height={24} />
        </div>
      </div>
    </div>
  ),
...
export const Interactive: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Interactive sparklines with hover tooltips showing exact values.',
      },
    },
  },
}
...
export const EdgeCases: Story = {
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4">
          ...
          <div className="flex items-center justify-between p-3 border rounded">
            ...
            <Sparkline data={[50]} width={80} height={24} color="text-blue-600" />
          </div>
          
          <div className="flex items-center justify-between p-3 border rounded">
            <span className="text-sm">All same values:</span>
            <Sparkline data={[100, 100, 100, 100, 100]} width={80} height={24} color="text-green-600" />
          </div>
          
          <div className="flex items-center justify-between p-3 border rounded">
            <span className="text-sm">High volatility:</span>
            <Sparkline data={[10, 90, 5, 95, 15, 85, 20, 80]} width={80} height={24} color="text-red-600" />
          </div>
          
          <div className="flex items-center justify-between p-3 border rounded">
            <span className="text-sm">Trending up:</span>
            <Sparkline data={[20, 25, 30, 35, 40, 45, 50, 55]} width={80} height={24} color="text-green-600" gradient />
          </div>
          
          <div className="flex items-center justify-between p-3 border rounded">
            <span className="text-sm">Trending down:</span>
            <Sparkline data={[80, 75, 70, 65, 60, 55, 50, 45]} width={80} height={24} color="text-red-600" gradient />
          </div>
        </div>
      </div>
    </div>
  ...
  parameters: {
    docs: {
      description: {
        story: 'Various edge cases and data patterns that the sparkline handles gracefully.',
      },
    },
  },
}
...
export const RealTimeData: Story = {
  render: () => {
    const [data, setData] = React.useState([120, 135, 158, 142, 167, 189, 203, 178])
    
    React.useEffect(() => {
      const interval = setInterval(() => {
        setData(prev => {
          const newData = [...prev.slice(1), Math.floor(Math.random() * 100) + 100]
          return newData
        })
      }, 1000)
  ...
  parameters: {
    docs: {
      description: {
        story: 'Simulated real-time data updates showing how sparklines handle dynamic data.',
      },
    },
  },
}
